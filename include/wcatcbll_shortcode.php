...
if (!function_exists('wcatcbll_shortcode')){
    function wcatcbll_shortcode($atts = array())    {
        ...
        */
        ...
        <style>
        <?php
            echo '.catcbll_preview_button{text-align:left;margin:'.$btn_margin.'}';
            echo '.catcbll_preview_button .fa{font-family:FontAwesome !important}';
            echo '.' . $catcbll_hide_btn_bghvr.':before{border-radius:'.$catcbll_btn_radius.'px;background:'.$catcbll_btn_hvrclr.';color:#fff;}'; 
            echo '.catcbll_preview_button .catcbll{color:'.$font_color.';font-size:'.$font_size.'px;padding:'.$catcbll_padding_top_bottom.'px '.$catcbll_padding_left_right.'px;border:'.$border_size.'px solid '.$border_color.';border-radius:'.$catcbll_btn_radius.'px;background:'.$background.';}';
            echo '.catcbll_preview_button a{text-decoration: none !important;}';
            if(empty($catcbll_hide_btn_bghvr)){
                echo '.catcbll:hover{border-radius:'.$catcbll_btn_radius.';background:'.$catcbll_btn_hvrclr.';color:#fff;}';
            }
        ?>
            .quantity, .buttons_added {display: inline-block;} .stock{display: none}
        </style>
        <?php
        if ($pid_count > 0){
                        ...
                        if (!empty($prd_lbl[0]) && ($custom == "custom")){
                            ...
                            for ($y = 0;$y < $atxtcnt;$y++){
                                if ($icon_position == 'right'){                        
                                    if (!empty($prd_lbl[$y])){                          
                                        $prd_btn = '<div class="catcbll_preview_button"><a href="' . $prd_url[$y] . '" class="btn btn-lg catcbll '.$catcbll_hide_btn_bghvr.' '.$catcbll_hide_2d_trans.'" ' . $trgtblnk . '>' . $prd_lbl[$y] . ' <i class="fa ' . $font_awesome .'"></i></a></div>';  
                                    }
                                }else{
                                    //Checking label field .It is empty or not
                                    if (!empty($prd_lbl[$y])){
                                        $prd_btn = '<div class="catcbll_preview_button"><a href="' . $prd_url[$y] . '" class="btn btn-lg catcbll '.$catcbll_hide_btn_bghvr.' '.$catcbll_hide_2d_trans.' " ' . $trgtblnk . '><i class="fa ' . $font_awesome .'"></i> ' . $prd_lbl[$y] . ' </a></div>'; 
                                    }
                                }
                                echo $prd_btn;
                            } // end for loop
