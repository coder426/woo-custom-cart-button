...
class wccb_widget extends WP_Widget{
    ...
    public function widget($args, $instance){
		...
		<style>
                    <?php
                        echo '.widget_wccb_widget .catcbll_preview_button{text-align:left;margin:'.$btn_margin.'}';
                        echo '.widget_wccb_widget .catcbll_preview_button .fa{font-family:FontAwesome !important}';
                        echo '.widget_wccb_widget .' . $catcbll_hide_btn_bghvr.':before{border-radius:'.$catcbll_btn_radius.'px;background:'.$catcbll_btn_hvrclr.';color:#fff;}'; 
                        echo '.widget_wccb_widget .catcbll_preview_button .catcbll{color:'.$catcbll_btn_fclr.'!important;font-size:'.$catcbll_btn_fsize.'px;padding:'.$catcbll_padding_top_bottom.'px '.$catcbll_padding_left_right.'px;border:'.$catcbll_border_size.'px solid '.$catcbll_btn_border_clr.';border-radius:'.$catcbll_btn_radius.'px;background:'.$catcbll_btn_bg.';}';
                        echo '.widget_wccb_widget .catcbll_preview_button a{text-decoration: none !important;}';
                        if(empty($catcbll_hide_btn_bghvr)){
                            echo '.widget_wccb_widget .catcbll:hover{border-radius:'.$catcbll_btn_radius.';background:'.$catcbll_btn_hvrclr.';color:#fff;}';
                        }
                    ?> 
                        .widget_wccb_widget .quantity,.widget_wccb_widget .buttons_added {display: inline-block;}.widget_wccb_widget .stock{display: none}
                    </style>
<?php
                ...
                if (!empty($prd_lbl[0]) && ($custom == "custom")){
                    ...
                    for ($y = 0;$y < $atxtcnt;$y++){                       
                        if ($catcbll_btn_icon_psn == 'right'){                        
                            if (!empty($prd_lbl[$y])){                          
                                $prd_btn = '<div class="catcbll_preview_button"><a href="' . $prd_url[$y] . '" class="btn btn-lg catcbll '.$catcbll_hide_btn_bghvr.' '.$catcbll_hide_2d_trans.'" ' . $trgtblnk . '>' . $prd_lbl[$y] . ' <i class="fa ' . $catcbll_btn_icon_cls .'"></i></a></div>';  
                            }
                        }else{
                            //Checking label field .It is empty or not
                            if (!empty($prd_lbl[$y])){
                                $prd_btn = '<div class="catcbll_preview_button"><a href="' . $prd_url[$y] . '" class="btn btn-lg catcbll '.$catcbll_hide_btn_bghvr.' '.$catcbll_hide_2d_trans.' " ' . $trgtblnk . '><i class="fa ' . $catcbll_btn_icon_cls .'"></i> ' . $prd_lbl[$y] . ' </a></div>'; 
                            }
                        }
                        echo $prd_btn;
                    } //end for
    ...
    public function form($instance){
        if (isset($instance['title'])){ $title = $instance['title'];}else{$title = __('New title', 'catcbll');}
        if (isset($instance['nprd'])){$nbr_prd = $instance['nprd'];}else{$nbr_prd = '';}
        ?>
        <p>
